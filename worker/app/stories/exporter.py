...
def process_stories_export(
    ...
    3. Render each slide to PNG via Chromium
    4. Compile PNGs to MP4 via FFmpeg
    5. Zip PNGs
    6. Upload ZIP + MP4 to MinIO
    7. Update story_export_jobs to done
...
def _fetch_trip(trip_id: str, database_url: str, minio_public_endpoint: str) -> Any:
    ...
    with psycopg.connect(database_url) as conn:
        with conn.cursor() as cur:
            # Fetch trip
            cur.execute(
                "SELECT id, name, cover_image_url FROM trips WHERE id = %s", (trip_id,)
            )
            row = cur.fetchone()
            if not row:
                raise RuntimeError(f"Trip {trip_id} not found")
            trip = SimpleNamespace(
                id=row[0], name=row[1], cover_image_url=row[2], days=[]
            )
            ...
            for day_id, day_number, date, notes in day_rows:
                ...
                city = loc_row[0] if loc_row else None
                country = loc_row[1] if loc_row else None
                ...
                for act_id, title, location, scheduled_time, act_status in act_rows:
                    ...
                    # Fetch photo memories for this activity
                    cur.execute(
                ...
                # Fetch day-level memories (no activity_id) — photos and videos
                cur.execute(
                    "SELECT id, memory_type, storage_key, caption FROM memories "
                    "WHERE day_id = %s AND activity_id IS NULL",
                    (day_id,),
                )
                for mem_id, mem_type, storage_key, caption in cur.fetchall():
                    public_url = (
